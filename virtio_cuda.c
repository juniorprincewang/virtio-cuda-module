...
static struct sg_table *get_sg_table(unsigned long addr, size_t size)
{
	struct page **page_list;
	struct sg_table *st;
	int nr_pages=0;
	...
	page_list = uaddr_to_pages(addr, size, 1);
	if(!page_list) {
		// cudaErrorMemoryAllocation             =      2,
		pr_err("Failed to get page list.\n");
		return NULL;
	}
	nr_pages = page_nr(addr, size);
	...
	gldebug("st->nents 0x%x\n", st->nents);
	return st;
}

static struct sg_table *get_malloc_sg_table(size_t size)
{
	...
	size_left = size;
	page_nr = PAGE_ALIGN(size)>>PAGE_SHIFT;
	st = kmalloc(sizeof(*st), GFP_KERNEL);
	if(!st)
		return NULL;
	if(sg_alloc_table(st, page_nr, GFP_KERNEL)) {
		kfree(st);
		return NULL;
	}
	sg = st->sgl;
...
static void copy_to_user_sg_table(unsigned long __user uaddr, size_t size, struct sg_table *st)
{
	unsigned long size_left = 0, offset = 0;
	void *addr;
	struct scatterlist *sg;
	int i =0;
	...
	for_each_sg(st->sgl, sg, st->nents, i) {
		if(sg_page(sg)) {
			...
			if (copy_to_user((void*)uaddr+offset, addr, sg->length)) {
...
static struct sg_table *copy_to_sg_table(unsigned long uaddr, size_t size)
{
	unsigned long size_left;
	struct sg_table *st;
	struct scatterlist *sg;
	...
	while(size_left) {
		...
		st->nents++;
		// gldebug("block_size 0x%lx, order %d, nents 0x%x\n", block_size, order, st->nents);
		if(copy_from_user(page_to_virt(page), (void __user *)uaddr+offset, block_size))
		{
			pr_err("Failed to copy from user. offset 0x%lx, size 0x%lx\n",
						offset, block_size);
			goto error;
		}
		sg_set_page(sg, page, block_size, 0);
...
static void free_malloc_sg_table(struct sg_table *st)
{
	struct scatterlist *sg;
	int i =0;
...
/*	
	...
	if(!(st = get_sg_table(payload->src, payload->srcSize))) {
		gldebug("overflow\n");
		overflow = 1;
		if(!(st = copy_to_sg_table(payload->src, payload->srcSize))) {
			pr_err("Failed to copy to sg table.\n");
			kfree(payload);
			return -ENOMEM;
		}
	}
...
{
	VirtIOArg *payload;
	struct scatterlist *sgs[3], arg_sg, mac_sg;
	int num_out = 0, num_in = 0;
	struct sg_table *st;
	int overflow = 0;
	void *macs=NULL;
...
static int cuda_memcpy_dtoh(VirtIOArg __user *arg, struct port *port)
{
	...
	if(!(st = get_sg_table(payload->dst, payload->dstSize))) {
		pr_err("Pages num exceed %d\n", VIRTIO_INDIRECT_NUM_MAX);
		overflow = 1;
		if(!(st = get_malloc_sg_table(payload->dstSize))) {
			pr_err("Failed to copy to sg table.\n");
			kfree(payload);
			return -ENOMEM;
		}
	...
	if(overflow) {
		copy_to_user_sg_table(payload->dst, payload->dstSize, st);
	}
	if(macs)
		kfree(macs);
	kfree(payload);
...
static int send_src_payload(VirtIOArg __user *arg, struct port *port)
{
	VirtIOArg *payload;
	void *ptr;
	struct scatterlist *sgs[2], arg_sg, src_sg;
	...
	sg_init_one(&arg_sg, payload, sizeof(*payload));
	sgs[num_out++] = &arg_sg;
	sg_init_one(&src_sg, ptr, payload->srcSize);
...
static int cublas_create(VirtIOArg __user *arg, struct port *port)
{
	func();
	return send_src_payload(arg, port);
}
...
static int send_cublas_set_payload(VirtIOArg __user *arg, struct port *port)
{
	VirtIOArg *payload;
	void *ptr = NULL;
	void *h_mem = NULL;
	struct scatterlist *sgs[3], arg_sg, src_sg, buf_sg;
	int num_out = 0, num_in = 0;
	...
	h_mem = memdup_user((const void __user *)payload->src, payload->srcSize);
	if(!h_mem) {
		pr_err("[ERROR] can not malloc 0x%x memory\n", payload->srcSize);
		return -ENOMEM;
	}
	sg_init_one(&arg_sg, payload, sizeof(*payload));
	sgs[num_out++] = &arg_sg;
	sg_init_one(&buf_sg, ptr, payload->paramSize);
	sgs[num_out++] = &buf_sg;
	sg_init_one(&src_sg, h_mem, payload->srcSize);
	sgs[num_out++] = &src_sg;
	send_sgs_to_virtio(port, sgs, num_out, num_in);
...
static int cublas_set_vector(VirtIOArg __user *arg, struct port *port)
{
	func();
	return send_cublas_set_payload(arg, port);
}

static int send_cublas_get_payload(VirtIOArg __user *arg, struct port *port)
{
	VirtIOArg *payload;
	void *ptr = NULL;
	void *h_dst = NULL;
	struct scatterlist *sgs[3], arg_sg, buf_sg, dst_sg;
	int num_out = 0, num_in = 0;
	...
	h_dst = kmalloc(payload->srcSize, GFP_KERNEL);
	if(!h_dst) {
		pr_err("[ERROR] can not malloc 0x%x memory\n", payload->srcSize);
		return -ENOMEM;
	}
	...
	sg_init_one(&buf_sg, ptr, payload->paramSize);
	sgs[num_out++] = &buf_sg;
	sg_init_one(&dst_sg, h_dst, payload->srcSize);
	sgs[num_out+num_in++] = &dst_sg;
	send_sgs_to_virtio(port, sgs, num_out, num_in);
	...
	gldebug("[+] arg->cmd = %d\n", payload->cmd);
	copy_to_user(arg, payload, ARG_SIZE);
	copy_to_user((void __user *)payload->dst, h_dst, payload->srcSize);
	kfree(ptr);
	kfree(h_dst);
	kfree(payload);
	return 0;
...
{
	func();
	return send_cublas_get_payload(arg, port);
}
...
static int cublas_asum(VirtIOArg __user *arg, struct port *port)
{
	...
	h_dst = kmalloc(payload->dstSize, GFP_KERNEL);
	if(!h_dst) {
		pr_err("[ERROR] can not malloc 0x%x memory\n", payload->dstSize);
		return -ENOMEM;
	}
	...
	sg_init_one(&arg_sg, payload, sizeof(*payload));
	sgs[num_out++] = &arg_sg;
	sg_init_one(&dst_sg, h_dst, payload->dstSize);
	sgs[num_out+num_in++] = &dst_sg;
	send_sgs_to_virtio(port, sgs, num_out, num_in);
	...
	gldebug("[+] arg->cmd = %d\n", payload->cmd);
	copy_to_user(arg, payload, ARG_SIZE);
	copy_to_user((void __user *)payload->dst, h_dst, payload->dstSize);
	kfree(h_dst);
	kfree(payload);
	return 0;
}
...
static int cublas_dot(VirtIOArg __user *arg, struct port *port)
{
	VirtIOArg *payload;
	void *ptr = NULL;
	struct scatterlist *sgs[2], arg_sg, buf_sg;
	int num_out = 0, num_in = 0;
	...
	ptr = kmalloc(payload->paramSize, GFP_KERNEL);
	if(!ptr) {
		pr_err("[ERROR] can not malloc 0x%x memory\n", payload->paramSize);
		return -ENOMEM;
	}
...
int curand_generate(VirtIOArg __user *arg, struct port *port)
{
	...
	sgs[num_out++] = &arg_sg;
	if (payload->flag == 1) {
		ptr = kmalloc(payload->dstSize, GFP_KERNEL);
		if(!ptr) {
			pr_err("[ERROR] can not malloc 0x%x memory\n", payload->dstSize);
			return -ENOMEM;
		}
		sg_init_one(&buf_sg, ptr, payload->dstSize);
		sgs[num_out+num_in++] = &buf_sg;
	}
	send_sgs_to_virtio(port, sgs, num_out, num_in);
	...
	copy_to_user(arg, payload, ARG_SIZE);
	if(payload->flag == 1) {
		copy_to_user((void __user*)arg->dst, ptr, payload->dstSize);
		kfree(ptr);
	}
	kfree(payload);
	return 0;
...
int curand_generate_normal_v2(VirtIOArg __user *arg, struct port *port)
{
	...
	h_mem = memdup_user((const void __user *)payload->param, payload->paramSize);
	if(!h_mem) {
		pr_err("[ERROR] can not malloc 0x%x memory\n", payload->paramSize);
		return -ENOMEM;
	}
	sg_init_one(&arg_sg, payload, sizeof(*payload));
	sgs[num_out++] = &arg_sg;
	sg_init_one(&buf_sg, h_mem, payload->paramSize);
	sgs[num_out++] = &buf_sg;
	if (payload->flag == 1) {
		ptr = kmalloc(payload->dstSize, GFP_KERNEL);
		if(!ptr) {
