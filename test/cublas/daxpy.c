...
int main(void){
	...
	res=(double *)malloc (n*sizeof(*res));// host memory alloc for y
	...
	stat=cublasDaxpy(handle,n,&al,d_x,1,d_y,1);
	stat=cublasGetVector(n,sizeof(double),d_y,1,y,1);//cp d_y->y
	printf("y after Daxpy:\n"); // print y after Daxpy
	for(j=0;j<n;j++)
		printf("%g, ",y[j]);
	printf("\n");
	for(j=0;j<n;j++) {
